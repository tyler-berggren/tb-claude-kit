...
- **`/commit sub <name>`** — Commits all changes, then pushes the named subtree to its deploy repo.
...
1. **Determine scope** based on modifier (default = all, "only" = session files, "sub" = all + subtree push)
...
3. **Regenerate SQLite sidecars** (if configured)
   ...
   The script is a no-op if no `sqlite_tracking` is configured in `.claude/kit.json`. See the
   script header for config format.

4. **Generate commit message** summarizing the changes

5. **Stage and commit**

   Read the optional author override first — it applies to all modes:
   ```bash
...
6. **Subtree push** (only with `sub` modifier)

   After committing, push the named subtree to its deploy repo. Read the subtree config from
   `.claude/kit.json`:
