...
**Everything you are pointed to comes with its line.** Plans grow long. So every question,
review item, finding or resume point Claude hands you names its file and line, as a link you
click straight through. The number is looked up just before the message goes out, because the
plan keeps changing during a session. Inside the plan itself, parts are named by stable ids
(`Q3`, review item `#4`, a slice id), since the next edit would move any line number written
there.
