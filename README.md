...
Track changes to SQLite databases through git by generating NDJSON sidecar files — one JSON line per row, sorted by rowid. Git tracks lines, not binary blobs, so each row change produces a clean, readable diff.
...
**What it buys:**
- `git log -p -- prospects.ndjson` — what each run found, what changed
- `git blame prospects.ndjson` — when each record entered and which run found it
